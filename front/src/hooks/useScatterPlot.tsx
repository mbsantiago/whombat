...
type Grouper = (annotation: Annotation) => GroupKey | null;

const defaultGrouper = () => "annotation";

const getFeatureValue = (annotation: Annotation, name: string) => {
  const feature = annotation.sound_event.features.find(
    (f) => f.feature_name.name === name,
  );
...
export default function useScatterPlot({
  ...
  const onClick = useCallback(
    (datum: Readonly<PlotMouseEvent>) => {
      ...
      onAnnotationClick?.(annotation as unknown as Annotation);
    },
    [onAnnotationClick],
  );
