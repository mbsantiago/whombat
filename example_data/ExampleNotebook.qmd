...
# Using Exported Data For Training Models
...
clip_status = {
    task.clip.uuid: task_is_complete(task) for task in annotation_project.tasks
}
...
# Filter the clip annotations to include only those that are complete and have no issues.
annotated_clips = [
    annotation
...
X_train = [get_features(annotation.sound_event) for annotation in train_sound_events]
```
...
```{python}
y_train = [
    data.find_tag(annotation.tags, "species").value for annotation in train_sound_events
]
```
...
predictions = [
    process_clip(clip_annotation) for clip_annotation in evaluation_set.clip_annotations
]
```
